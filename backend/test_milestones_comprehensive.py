...
class MilestoneTester:
    ...
    def setup(self):
        ...
        if not self.admin_token:
            print(
                f"{Colors.YELLOW}Warning: Could not login as admin. Some tests may fail.{Colors.END}"
            )
        ...
        if not self.token:
            print(
                f"{Colors.YELLOW}Warning: Could not login as test user. Creating one...{Colors.END}"
            )
        ...
        if self.admin_token:
            clients_resp = requests.get(
                f"{self.base_url}/clients", headers=self.get_headers(admin=True)
            )
            ...
            if self.test_client_id:
                projects_resp = requests.get(
                    f"{self.base_url}/projects", headers=self.get_headers(admin=True)
                )
    ...
    def test_1_milestone_crud(self):
        ...
        create_resp = requests.post(
            f"{self.base_url}/milestones", headers=self.get_headers(admin=True), json=create_data
        )

        if create_resp.status_code != 201:
            self.result.fail_test(
                "Create Milestone", f"Status {create_resp.status_code}: {create_resp.text}"
            )
            return None
        ...
        get_resp = requests.get(
            f"{self.base_url}/milestones/{milestone_id}", headers=self.get_headers(admin=True)
        )
    ...
    def test_3_task_linking(self, milestone_id: Optional[int]):
        ...
        task_resp = requests.post(
            f"{self.base_url}/tasks", headers=self.get_headers(admin=True), json=task_data
        )
        ...
        if link_resp.status_code != 200:
            self.result.fail_test(
                "Link Task to Milestone", f"Status {link_resp.status_code}: {link_resp.text}"
            )
            return task_id
        ...
        if get_resp.status_code == 200:
            ...
            if progress:
                ...
                if completed >= 1:  # At least one done task
                    ...
                    self.result.fail_test(
                        "Progress Calculation", f"Expected completed tasks, got {completed}"
                    )
                ...
                if percentage >= 0 and percentage <= 100:
                    ...
                    self.result.fail_test(
                        "Progress Calculation", f"Invalid percentage: {percentage}"
                    )
            else:
                self.result.fail_test("Progress Calculation", "Progress object missing")
        else:
            self.result.fail_test(
                "Progress Calculation", f"Cannot fetch milestone: {get_resp.status_code}"
            )
    ...
    def test_7_milestone_deletion_safety(self, milestone_id: Optional[int], task_id: Optional[int]):
        ...
        if task_id:
            task_resp = requests.get(
                f"{self.base_url}/tasks/{task_id}", headers=self.get_headers(admin=True)
            )
            if task_resp.status_code == 200:
                task = task_resp.json()
                if task.get("milestone_id") is None:
                    self.result.pass_test(
                        "Task Survives Milestone Deletion (milestone_id set to NULL)"
                    )
                else:
                    self.result.fail_test("Task Deletion Safety", "Task milestone_id not cleared")
            else:
                self.result.fail_test(
                    "Task Deletion Safety", "Task not found after milestone deletion"
                )
