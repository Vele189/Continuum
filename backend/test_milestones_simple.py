...
def check_database_schema():
    ...
    try:
        ...
        if cur.fetchone():
            ...
            # Check index
            cur.execute("SELECT name FROM sqlite_master WHERE type='index' AND name LIKE 'ix_milestones%'")
...
def check_permissions_implementation():
    ...
    if os.path.exists(milestones_routes):
        with open(milestones_routes, 'r') as f:
            ...
            has_project_check = "project" in content.lower() and ("member" in content.lower() or "admin" in content.lower())
