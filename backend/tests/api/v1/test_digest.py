...
def test_get_project_digest_successful(client: TestClient, db: Session):
    ...
    test_client = Client(name="Test Client", email="client@test.com", created_by=admin_user.id)
    db.add(test_client)
    db.commit()
...
def test_get_project_digest_forbidden(client: TestClient, db: Session):
    ...
    # Create a client first
    test_client = Client(name="Test Client", email="client@test.com", created_by=user1.id)
    db.add(test_client)
    db.commit()
    db.refresh(test_client)

    # Project for user 1
    project = Project(name="User 1 Project", client_id=test_client.id)
    db.add(project)
    db.commit()
    db.refresh(project)
