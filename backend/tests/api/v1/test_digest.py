...
def test_get_project_digest_successful(client: TestClient, db: Session):
    ...
    response = client.get(
        f"{settings.API_V1_STR}/projects/{project.id}/digest",
        params={"week_start": "2023-10-01"}
    )
...
def test_get_project_digest_forbidden(client: TestClient, db: Session):
    # Setup two users
    user1 = User(
        email="user1@test.com", username="user1", display_name="User 1",
        first_name="U1", last_name="S1", hashed_password="pw", role=UserRole.BACKEND
    )
    user2 = User(
        email="user2@test.com", username="user2", display_name="User 2",
