...
def test_client_portal_project_read(client: TestClient):
    ...
    portal_client = Client(name="Portal Client", api_key="client-portal-token")
    ...
    member = ProjectMember(project_id=project.id, user_id=user.id, role="developer")
    db.add(member)
    db.commit()
    ...
    response = client.get(f"{settings.API_V1_STR}/client-portal/projects/{project_id}")
    assert response.status_code == 401
...
def test_client_portal_sanitization(client: TestClient):
    ...
    project = Project(name="Secrets Project", client_id=portal_client.id)
    db.add(project)
    db.commit()
    db.refresh(project)
    project_id = project.id
