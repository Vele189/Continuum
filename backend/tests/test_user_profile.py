...
def login_user(client: TestClient, email: str):
    ...
    response = client.post(
        ...
        },  # Note: we need to set the password to "password123" in create_user
    )
...
def test_contributions_summary_aggregation(client: TestClient):
    ...
    # Create a project
    project = Project(
        name="Aggregation Project", client_id=1
    )  # Assume client 1 exists or create one
    db.add(project)
    db.commit()
    db.refresh(project)
    ...
    # Log some hours
    log1 = LoggedHour(user_id=user_id, project_id=project.id, hours=5.5, logged_at=datetime.now())
    log2 = LoggedHour(
        user_id=user_id,
    ...
    # Add a commit
    commit = GitContribution(
        user_id=user_id, project_id=project.id, commit_hash="abc12345", provider="github"
    )
    db.add(commit)
