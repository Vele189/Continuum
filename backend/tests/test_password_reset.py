...
def test_password_reset_flow(client: TestClient):
    ...
    client.post(
        ...
        json={"email": email, "password": old_password, "first_name": "Reset", "last_name": "Test"},
    )
    ...
    login_response = client.post(
        f"{settings.API_V1_STR}/auth/login", json={"email": email, "password": new_password}
    )
    assert login_response.status_code == 200

    # 6. Verify Old Password Fails
    fail_response = client.post(
        f"{settings.API_V1_STR}/auth/login", json={"email": email, "password": old_password}
    )
