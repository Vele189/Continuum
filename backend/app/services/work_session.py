...
from app.dbmodels import LoggedHour, User, WorkSession, WorkSessionStatus
...
def pause_session(db: Session, session_id: int, user: User) -> WorkSession:
    ...
    if session.status != WorkSessionStatus.ACTIVE:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Only active sessions can be paused"
        )
...
def resume_session(db: Session, session_id: int, user: User) -> WorkSession:
    ...
    if session.status != WorkSessionStatus.PAUSED:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Only paused sessions can be resumed"
        )
...
def stop_session(db: Session, session_id: int, user: User) -> WorkSession:
    ...
    if session.status not in [WorkSessionStatus.ACTIVE, WorkSessionStatus.PAUSED]:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Session is already completed"
        )
...
def list_sessions(db: Session, user_id: int, skip: int = 0, limit: int = 100) -> List[WorkSession]:
