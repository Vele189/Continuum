...
from app.database import User, Project, GitContribution
...
class WebhookService:
    ...
    def _extract_github_commits(
        ...
        for commit in payload.commits:
            ...
            commits.append(CommitInfo(
                ...
                branch=branch,
                timestamp=WebhookService._normalize_timestamp(
                    timestamp_str,
                    "github"
                ),
    ...
    @staticmethod
    def _extract_bitbucket_commits(
    ...
    def process_github_push(
        ...
        if not repository_url and payload.repository:
            repository_url = (
                payload.repository.get("clone_url") or
                payload.repository.get("html_url")
            )
        ...
        if not project:
            logger.warning(
                "No project mapping found for repository: %s",
                repository_name or repository_url
            )
            raise HTTPException(
        ...
        # Process commits
        return WebhookService._process_commits(
            db,
            commits,
            project.id,
            "github",
    ...
    def process_gitlab_push(
        ...
        if not repository_url and payload.repository:
            repository_url = (
                payload.repository.get("git_http_url") or
                payload.repository.get("url")
            )
        if not repository_name and payload.project:
            repository_name = (
                payload.project.get("path_with_namespace") or
    ...
    def _process_commits(
        ...
        for commit in commits:
            try:
                ...
                # Find user by email
                user = WebhookService._find_user_by_email(
                    db,
                    commit.author_email
                )
                ...
                if existing:
                    logger.debug(
                        "Skipping duplicate commit %s (already exists)",
                        commit.hash[:8]
                    )
                    skipped_count += 1
                    continue
