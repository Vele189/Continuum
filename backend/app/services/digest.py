...
from app.dbmodels import Project, Task, LoggedHour, GitContribution, User
from app.services.project import ProjectService
from app.schemas.digest import WeeklyDigest, TaskSummary, UserHours, CommitSummary, RiskItem
...
class DigestService:
    ...
    def generate_weekly_digest(
        ...
        # 2. Completed Tasks
        completed_tasks_query = db.query(Task).filter(
        ...
        ).join(User, LoggedHour.user_id == User.id).filter(
            LoggedHour.project_id == project_id,
            LoggedHour.logged_at >= week_start,
            LoggedHour.logged_at < week_end
        ).group_by(LoggedHour.user_id, User.display_name).all()

        total_hours_logged = sum(h.total_hours for h in hours_query)
        hours_breakdown = [
            UserHours(
                user_id=h.user_id,
                user_name=h.display_name,
                hours=h.total_hours
            ) for h in hours_query
        ]

        # 4. Git Contributions
        commits_query = db.query(GitContribution).filter(
        ...
        milestone_progress = {
            "note": "Milestone progress tracking to be implemented in phase 2"
        }

        # 6. Risks & Delays (Placeholder Only)
        risks_and_delays = [
            RiskItem(
                reason="Risk detection not implemented yet",
                severity="info"
            )
        ]
