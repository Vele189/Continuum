...
from app.dbmodels import Client, Invoice, Project
...
def _check_weasyprint():
    ...
    if WEASYPRINT_AVAILABLE is None:
        try:
            import weasyprint  # noqa: F401, C0415  # pylint: disable=unused-import
...
def _check_reportlab():
    ...
    if REPORTLAB_AVAILABLE is None:
        try:
            import reportlab.lib  # noqa: F401, C0415  # pylint: disable=unused-import
...
def _generate_pdf_reportlab(
    ...
    for item in invoice.items:
        ...
        items_data.append([date_para, resource_para, desc_para, hours_para, rate_para, amount_para])

    # Adjust column widths for better layout
    items_table = Table(
        items_data, colWidths=[0.9 * inch, 1.4 * inch, 2.2 * inch, 0.6 * inch, 0.9 * inch, 1 * inch]
    )
    ...
    story.append(Paragraph(f"Generated on {_format_date(invoice.created_at)}", styles["Normal"]))
    story.append(
        Paragraph("This is an automatically generated invoice from Continuum", styles["Normal"])
    )
...
def generate_invoice_pdf(
    ...
    if _check_weasyprint():
        try:
            logger.info("Generating PDF for invoice %s using WeasyPrint", invoice.invoice_number)
            return _generate_pdf_weasyprint(invoice, project, client)
        except Exception as e:
            logger.warning("WeasyPrint PDF generation failed: %s, falling back to ReportLab", e)
            if _check_reportlab():
                return _generate_pdf_reportlab(invoice, project, client)
            raise RuntimeError(f"PDF generation failed: {e}") from e
    elif _check_reportlab():
        logger.info("Generating PDF for invoice %s using ReportLab", invoice.invoice_number)
        return _generate_pdf_reportlab(invoice, project, client)
    else:
        raise RuntimeError("Neither WeasyPrint nor ReportLab is available for PDF generation")


def save_invoice_pdf(invoice: Invoice, project: Project, client: Optional[Client] = None) -> str:
    """
